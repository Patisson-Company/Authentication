...
from tokens.jwt import check_token, create_refresh_token, create_service_token, tokens_up
...
async def create_route(
    ...
    with tracer.start_as_current_span("service-authentication") as span:
        ...
        async with get_session() as session:  # type: ignore[reportAssignmentType]
            is_valid, body = await service_auth(
                session, login, password  # type: ignore[reportAssignmentType]
            )
        ...
        if not is_valid:
            ...
            raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail=[body.model_dump()])
...
async def verify_route(
    ...
    with tracer.start_as_current_span("verify") as span:
        span.set_attribute("service.verified_service_jwt", mask_token(str(verified_service_jwt)))

        is_valid, body = check_token(
            token=str(verified_service_jwt), schema=ServiceAccessTokenPayload, carrier=TokenBearer.SERVICE
        )
