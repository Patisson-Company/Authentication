...
from patisson_request.jwt_tokens import ClientAccessTokenPayload, TokenBearer, mask_token
from patisson_request.roles import ClientRole
from patisson_request.service_requests import AuthenticationRequest
from patisson_request.service_responses import AuthenticationResponse, TokensSetResponse
from tokens.jwt import check_token, create_client_token, create_refresh_token, tokens_up
...
async def create_route(
    ...
    return TokensSetResponse(access_token=access_token, refresh_token=refresh_token)
...
async def verify_route(
    ...
    with tracer.start_as_current_span("verify") as span:
        ...
        is_valid, body = check_token(
            token=str(client_access_token), schema=ClientAccessTokenPayload, carrier=TokenBearer.CLIENT
        )
        ...
        if is_valid:
            logger.info(
                (
                    f"the {body.sub} token is valid,"  # type: ignore[reportArgumentType]
                    f" the verifying service {service_jwt.sub}"
                )
            )
            return AuthenticationResponse.Verify(
                is_verify=is_valid, payload=body, error=None  # type: ignore[reportArgumentType]
            )
        else:
            logger.info(f"the token is not valid, the verifying service {service_jwt.sub}")
            return AuthenticationResponse.Verify(
...
async def update_route(
    ...
    with tracer.start_as_current_span("tokens-up") as span:
        span.set_attribute("client.passed_access_token", mask_token(str(client_access_token)))
        span.set_attribute("client.passed_refresh_token", mask_token(str(client_refresh_token)))
        span.set_attribute("token.expire_in", str(expire_in))
        ...
        if is_valid:
            span.set_attribute(
                "client.created_access_token",
                mask_token(body.access_token),  # type: ignore[reportAttributeAccessIssue]
            )
            span.set_attribute(
                "client.created_refresh_token",
