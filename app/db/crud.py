    ...
    try:
        ...
        if not service:
            return False, ErrorSchema(
                error=ErrorCode.INVALID_PARAMETERS, extra="the service login is incorrect"
            )
        ...
        if service and service.check_password(password):
            ...
            return False, ErrorSchema(
                error=ErrorCode.INVALID_PARAMETERS, extra="the service password is incorrect"
            )

    except ValueError:
        return False, ErrorSchema(error=ErrorCode.VALIDATE_ERROR, extra="the role is incorrect")
